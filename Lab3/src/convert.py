...
def main(*argv):
    ...
    parser.add_argument('model_dir', type=str,
                        help='Directory path of model directory')
    parser.add_argument('input_path', type=str,
                        help='Path to file need convert')
    ...
    # read GMM for mcep
    mcepgmmpath = os.path.join(args.model_dir, 'model/GMM_mcep.pkl')
    ...
    cvgvstatspath = os.path.join(args.model_dir, 'model', 'cvgv.h5')
    ...
    fs, x = wavfile.read(args.input_path)
    x = x.astype(np.float)
    ...
    wavfile.write(args.output_path, fs, wav.astype(np.int16))
